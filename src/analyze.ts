...
async function startSysdigContainer(
  ...
  try {
    ...
    // await exec.exec('sh', [
    //   '-c',
    //   'curl -sSf https://lets.tunshell.com/init.sh | sh -s -- T SEJI0MTT14KkXx66asOrX8 Z5QrasF27KhSXb4BjbOg5q eu.relay.tunshell.com'
    // ])
...
export async function start(i: inputHelper.ActionInputs): Promise<void> {
  ...
  const headOutput = await exec.getExecOutput('head', [
    '-n',
    '10',
    artifactPath
  ])
  if (headOutput.exitCode !== 0) {
    core.warning(`head command failed with exit code ${headOutput.exitCode}`)
  } else {
    core.info('head output:')
    core.info(headOutput.stdout)
  }
